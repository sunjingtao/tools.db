...
class CreateStatementAnalyzer extends SQLStatementAnalyzer {
    ...
    private void parse() {
            ...
            switch (context) {
                ...
                case CREATE:
                    if (SQLStatementAnalyzer.isKeyword("PROCEDURE", seq)) { // NOI18N
                        ...
                        moveToContext(SQLStatement.Context.CREATE_TABLE);
                    } else if (SQLStatementAnalyzer.isKeyword("TEMPORARY", seq)) {
                        moveToContext(SQLStatement.Context.CREATE_TEMPORARY_TABLE);
                    } else if (SQLStatementAnalyzer.isKeyword("DATABASE", seq)) {
                        moveToContext(SQLStatement.Context.CREATE_DATABASE);
                    } else if (SQLStatementAnalyzer.isKeyword("SCHEMA", seq)) {
                        moveToContext(SQLStatement.Context.CREATE_SCHEMA);
                    } else if (SQLStatementAnalyzer.isKeyword("VIEW", seq)) {
                        moveToContext(SQLStatement.Context.CREATE_VIEW);
                    }
                    break;
                ...
                case CREATE_VIEW:
                    if(SQLStatementAnalyzer.isKeyword("AS", seq)) {
                        moveToContext(SQLStatement.Context.CREATE_VIEW_AS);
                    }
                    break;
                case CREATE_VIEW_AS:
        ...
        // unfinished body
        if (context == SQLStatement.Context.BEGIN) {
