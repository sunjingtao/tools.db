...
package com.tools.data.db.sql.analyzer;

import com.tools.data.db.sql.lexer.SQLTokenId;
import com.tools.lib.lexer.TokenSequence;
import com.tools.data.db.core.SQLIdentifiers;
import com.tools.data.db.sql.analyzer.SQLStatement.Context;
...
class SelectStatementAnalyzer extends SQLStatementAnalyzer {
        ...
        main: for (;;) {
            ...
            switch (seq.token().id()) {
                ...
                case COMMA:
                    break main;
                case KEYWORD:
                    if (isKeyword("AS", seq)) { // NOI18N
                        // Alias will follow.
                        afterDot = false;
                        parts.clear();
                    } else if (isKeyword("FROM", seq) || isKeywordAfterFrom()) { // NOI18N
                        break main;
                    }
                    break;
                default:
    ...
    private Context getContextForKeywordAfterFrom() {
        if (isKeyword("ON", seq)) { // NOI18N
            return Context.JOIN_CONDITION;
        } else if (isKeyword("WHERE", seq)) { // NOI18N
            return Context.WHERE;
        } else if (isKeyword("GROUP", seq)) { // NOI18N
            return Context.GROUP;
        } else if (isKeyword("HAVING", seq)) { // NOI18N
            return Context.HAVING;
        } else if (isKeyword("ORDER", seq)) { // NOI18N
