 ...
 */
package com.tools.data.db.lib.jdbc;
...
class SQLStatement {
    ...
    static Logger logger = LoggerFactory.getLogger(SQLStatement.class);
    ...
    String executeQuery(String sql) {
        logger.info("executeQuery {0}", new Object[]{sql});
        return sql;
    }

    String executeUpdate(String sql) {
        logger.info("executeUpdate {0}", new Object[]{sql});
        return sql;
    }

    String execute(String sql) {
        logger.info("execute {0}", new Object[]{sql});
        return sql;
    }
    ...
    String executeBatch() {
        logger.info("executeBatch {0}", new Object[]{Arrays.toString(batch.toArray())});
        return Arrays.toString(batch.toArray());
    }

    void setNull(int parameterIndex, int sqlType) {
        logger.info( "setNull index:{0} type:{1}", new Object[]{parameterIndex,sqlType});
    ...
    void setDate(int parameterIndex, String x) {
        ...
        logger.info( "setDate index:{0} value:{1}", new Object[]{parameterIndex,new Date(time)});
    ...
    void setTimestamp(int parameterIndex, String x) {
        ensureCapacity(parameterIndex);
        long time = Long.valueOf(x);
        logger.info("setTime index:{0} value:{1}", new Object[]{parameterIndex,new Timestamp(time)});
    ...
    private void setParameter(String method, Integer parameterIndex, Object p) {
        logger.info("{0} index:{1} value:{2}", new Object[]{method,parameterIndex,p});
        ensureCapacity(parameterIndex);
        parameters.set(parameterIndex, p);
    }

    String executeQuery() {
        logger.info( "executeQuery {0}", new Object[]{getFullSql()});
        return getFullSql();
    }

    String executeUpdate() {
        logger.info( "executeUpdate {0}", new Object[]{getFullSql()});
        return getFullSql();
    }

    String execute() {
        logger.info("execute {0}", new Object[]{getFullSql()});
        return getFullSql();
    }
