...
public class Catalog extends Element{
    ...
    public Catalog(Metadata jdbcMetadata, String name, boolean _default, String defaultSchemaName) {
    ...
    protected Schema createJDBCSchema(String name, boolean _default, boolean synthetic) {
        return new Schema(this, name, _default, synthetic);
    }
    ...
    protected void createSchemas() {
        ...
        try {
                        ...
                        while (rs.next()) {
                            ...
                            if (MetadataUtilities.equals(catalogName, name)) {
                                if (defaultSchemaName != null && MetadataUtilities.equals(schemaName, defaultSchemaName)) {
                                    defaultSchema = createJDBCSchema(defaultSchemaName, true, false);
                                    newSchemas.put(defaultSchema.getName(), defaultSchema);
                                    LOGGER.log(Level.FINE, "Created default schema {0}", defaultSchema);
                                } else {
                                    Schema schema = createJDBCSchema(schemaName, false, false);
            ...
            if (newSchemas.isEmpty() && !jdbcMetadata.getDmd().supportsSchemasInTableDefinitions()) {
                syntheticSchema = createJDBCSchema(null, _default, true);
                if (_default) {
    ...
    public final Metadata getJDBCMetadata() {
        return jdbcMetadata;
    }
}
